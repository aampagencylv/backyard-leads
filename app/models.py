...
class User(TenantMixin, Base):
    ...
    last_brief_sent_at = Column(DateTime(timezone=True), nullable=True)
...
class Company(TenantMixin, Base):
    ...
    similarweb_fetched_at = Column(DateTime(timezone=True), nullable=True)
    ...
    tech_stack_fetched_at = Column(DateTime(timezone=True), nullable=True)
    # Yelp profile cache — populated on-demand via "Yelp" button.
    yelp_json = Column(Text, nullable=True)
    yelp_fetched_at = Column(DateTime(timezone=True), nullable=True)
    # Indeed hiring activity — populated on-demand.
    indeed_jobs_json = Column(Text, nullable=True)
    indeed_jobs_fetched_at = Column(DateTime(timezone=True), nullable=True)
    ...
    reviews_fetched_at = Column(DateTime(timezone=True), nullable=True)
    ...
    lead_score_updated_at = Column(DateTime(timezone=True), nullable=True)
...
class Contact(TenantMixin, Base):
    ...
    phone_type_checked_at = Column(DateTime(timezone=True), nullable=True)
    ...
    missive_conversation_seen_at = Column(DateTime(timezone=True), nullable=True)
    ...
    posts_fetched_at = Column(DateTime(timezone=True), nullable=True)
    ...
    linkedin_profile_fetched_at = Column(DateTime(timezone=True), nullable=True)
...
class GeneratedEmail(TenantMixin, Base):
    ...
    sent_at = Column(DateTime(timezone=True), nullable=True)  # when email was sent or task was completed
...
class RuntimeConfig(TenantMixin, Base):
    ...
    zoominfo_token_expires_at = Column(DateTime(timezone=True), nullable=True)
...
class SiteVisitorSession(TenantMixin, Base):
    ...
    first_seen_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False, index=True)
...
class Campaign(TenantMixin, Base):
    ...
    # When set, the campaign is archived — hidden from the main Auto
    # Pilot list. Used after a campaign has completed so the page stays
    # focused on what's actively running. Setting archived_at also
...
class AuditReportModel(TenantMixin, Base):
    ...
    competitor_generated_at = Column(DateTime(timezone=True), nullable=True)
